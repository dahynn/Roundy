...
public class StageScheduler {
    ...
    private void determineAndStartNextStage(RoomState room) {
        ...
        if (currentStage == Stage.SELF_INTRO && !isRepeating) needBreak = true;
        else if (currentStage == Stage.VOTE_FIRST) needBreak = true;
        else if (currentStage.isRotationStage()) needBreak = true;
        else if (currentStage == Stage.VOTE_FINAL) needBreak = true;
        else if (currentStage == Stage.MATCHING_RESULT) needBreak = true;
