...
import RoundyCamera, { RoundyCameraHandle } from '../../components/common/RoundyCamera';

const RotationTestPage: React.FC = () => {
    const roomId = "001";
    const roundyCameraRef = useRef<RoundyCameraHandle>(null);
    ...
    useEffect(() => {
        ...
        if (partnerInfo && partnerInfo.sessionId && partnerInfo.token) {
            ...
            // 📍 AI Masking Stream 추출
            const canvasStream = roundyCameraRef.current?.getCanvasStream();
            const videoTrack = canvasStream?.getVideoTracks()[0];

            if (!videoTrack) {
                console.warn("⚠️ RoundyCamera 스트림을 찾을 수 없습니다. 기본 웹캠을 사용합니다.");
            }
    ...
    return (
        <div style={{ padding: '20px', fontFamily: 'sans-serif', maxWidth: '1200px', margin: '0 auto' }}>
            <header style={{ marginBottom: '20px', borderBottom: '1px solid #ddd', paddingBottom: '10px' }}>
                <h1>🎥 로테이션 화상 테스트 (AI Masking Applied)</h1>
                <div>
                ...
                <div style={{ flex: 3 }}>
                    <div style={{ background: '#222', padding: '20px', borderRadius: '12px', minHeight: '500px' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', justifyContent: 'center' }}>
                            {/* 내 화면 (AI RoundyCamera) */}
                            <div style={{ border: '2px solid #ffff00', width: '320px', height: '240px', position: 'relative' }}>
                                <RoundyCamera ref={roundyCameraRef} />
                                <div style={{ position: 'absolute', bottom: 10, left: 10, background: 'rgba(0,0,0,0.5)', color: '#fff', padding: '2px 8px', borderRadius: '4px', fontSize: '12px' }}>
                                    {userProfile.username} (나)
                                </div>
                            </div>

                            {/* 상대방 화면들 (Subscribers) - 기존 방식 유지 */}
