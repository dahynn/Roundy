...
export const Step1_Intro = ({ participants, activeSpeakerIdx, publisher }: any) => {
      ...
      <div className="grid grid-cols-3 gap-6">
        {participants.slice(0, 3).map((p: any, idx: number) => (
          <SpeakerCard
            ...
            // For now, only show video if we have a streamManager mapped in container
            streamManager={p.streamManager}
          // If this participant is ME (based on logic), use publisher? 
          // For now assume logic handles streamManager mapping for remote, 
          // BUT for 'ME', we might need to pass publisher separately if p.id === myId
          />
        ))}
      </div>
      <div className="grid grid-cols-3 gap-6">
        {participants.slice(3, 6).map((p: any, idx: number) => (
          <SpeakerCard key={p.id} participant={p} isActive={activeSpeakerIdx === idx + 3} streamManager={p.streamManager} />
        ))}
      </div>
    </div>
  );
...
const SpeakerCard = ({ participant, isActive, streamManager }: any) => (
  <div
  ...
  >
    ...
    <div className="absolute top-6 left-8 flex items-center gap-2 bg-black/40 px-3 py-1 rounded-full backdrop-blur-md">
      <div className={`w-2 h-2 rounded-full ${isActive ? 'bg-[#FF4D94]' : 'bg-white/20'}`} />
      <span className="text-xs font-black text-white">{participant.name}</span>
    </div>
  </div>
);
